                        ...
                        <div class="chat-message-list" data-simplebar>
                            ...
                            <ul id="accounts" class="list-unstyled chat-list chat-user-list">
                                ...
                                <c:forEach items="${receiver}" var="account" varStatus="i">
                                    <li>
                                        <c:if test="${role eq 'user'}">
                                            <a class="wicket" phone="${account.tPhone}">
                                        </c:if>
                                        <c:if test="${role eq 'teacher'}">
                                            <a class="wicket" phone="${account.uPhone}">
                                        </c:if>
                                            <div class="media">
                                                <div class="chat-user-img align-self-center mr-3">
                                                    <c:if test="${role eq 'user'}">
                                                        <img src="${account.tPath}">
                                                    </c:if>
                                                    <c:if test="${role eq 'teacher'}">
                                                        <img src="${account.uPath}">
                                                    </c:if>
                                                </div>
                                                <div class="media-body overflow-hidden">
                                                    <c:if test="${role eq 'user'}">
                                                        <h5 class="text-truncate font-size-15 mb-1">${account.tName}</h5>
                                                    </c:if>
                                                    <c:if test="${role eq 'teacher'}">
                                                        <h5 class="text-truncate font-size-15 mb-1">${account.uName}</h5>
                                                    </c:if>
...
<%--                                </li>--%>
    ...
    <!-- Start User chat -->
    <!-- 聊天框 -->
    <div hidden="hidden" id="startChat" class="user-chat w-100">
        <div class="d-lg-flex">
                    ...
                    <div class="row align-items-center">
                        <div class="col-sm-4 col-8">
                            <div class="media align-items-center">
                                ...
                                <div id="headportrait" class="mr-3">
                                </div>
                                <div class="media-body overflow-hidden">
                                    <h5 class="font-size-16 mb-0 text-truncate"><a id="headportraitname" href="#" class="text-reset user-profile-show"></a> <i class="ri-record-circle-fill font-size-10 text-success d-inline-block ml-1"></i></h5>
                                </div>
                            </div>
                        </div>
...
<%--                        </li>--%>
                ...
                <div class="p-3 p-lg-4 border-top mb-0">
                    <div class="row no-gutters">
                        <div class="col">
                            <div>
                                <input id="content" type="text" class="form-control form-control-lg bg-light border-light" placeholder="输入消息...">
                            </div>
                        </div>
                        <div class="col-auto">
...
<script>
    $(function () {
        ...
        $(".wicket").click(function () {
            //显示聊天框
            $("#startChat").removeAttr("hidden");
            //移出用户的‘active’(class)
            $(".wicket").parent().attr('class',"");
            //给被选中的用户添加‘active’(class)
            ...
            $.getJSON("/chat.do?phoneStr="+phone,function (map) {
                ...
                if (map.role == "user") {
                    ...
                    $("#allcontent li").remove();
                    $.each(map.contacts,function (i,value) {
                        if (value.senderId != ${sessionScope.phone}){
                            var location = "<li class='left'>\n";
                            var locationPath = map.account.tPath;
                            var locationName = map.account.tName;
                        } else {
                            var location = "<li class='right'>\n";
                            var locationPath = '${sessionScope.user.uPath}';
                            var locationName = '${sessionScope.user.uName}';
                        }
                        $("#allcontent").append(
                            location +
                            "                            <div class=\"conversation-list\">\n" +
                ...
                if (map.role == "teacher") {
                    $("#headportrait").html("<img src='"+map.account.uPath+"' class='rounded-circle avatar-xs' alt=''>");
                    $("#headportraitname").text(map.account.uName);
                    $("#allcontent li").remove();
                    $.each(map.contacts,function (i,value) {
                        if (value.senderId != ${sessionScope.phone}){
                            var location = "<li class='left'>\n";
                            var locationPath = map.account.uPath;
                            var locationName = map.account.uName;
                        } else {
                            var location = "<li class='right'>\n";
                            var locationPath = '${sessionScope.teacher.tPath}';
                            var locationName = '${sessionScope.teacher.tName}';
                        }
                            $("#allcontent").append(
                                location +
                                "                            <div class=\"conversation-list\">\n" +
        ...
        $("#btn1").click(function () {
            ...
            if(content != null && content != ""){
                ...
                <c:if test="${user != null}">
                    locationPath = '${sessionScope.user.uPath}';
                    locationName = '${sessionScope.user.uName}';
                </c:if>
                <c:if test="${teacher != null}">
                    locationPath = '${sessionScope.teacher.tPath}';
                    locationName = '${sessionScope.teacher.tName}';
                </c:if>
                //聊天框显示发送内容
                $("#allcontent").append(
                    location +
        ...
        function receivemessages() {
            ...
            var phone = $("li[class='active'] a").attr("phone");
            $.getJSON("/receivemessages.do?time="+time+"&sphone="+phone,function (map) {
                if (map.contact != null && phone == map.contact.senderId) {
                    var locationPath = null;
                    var locationName = null;
                    if (map.role == "user") {
                        locationPath = map.account.uPath;
                        locationName = map.account.uName;
                    }
                    if (map.role == "teacher") {
                        locationPath = map.account.tPath;
                        locationName = map.account.tName;
                    }
                    $("#allcontent").append(
                        "<li class='left'>\n" +
                        "                            <div class=\"conversation-list\">\n" +
                ...
                //     var r = $(this);
                //     if(map.count[i] > 0){
                //         $(this).children("li:eq("+i+")").children(".unread-message span").show();
                //         $(this).children("li:eq("+i+")").children(".unread-message span").text(map.count[i]);
                //         $(this).children("li:eq("+i+")").children("p").text(map.lastContact[i].content);
                //     }
                // });
